...
mod limit_buy_hook {
    ...
    struct LimitBuyHook {
        ...
        order_books: KeyValueStore<ResourceAddress, OrderBook>,
    ...
    impl LimitBuyHook {
        ...
        pub fn new(
            ...
            // Reserve a component address to set proper permissions on the BuyOrder NFT
            let (address_reservation, component_address) = Runtime::allocate_component_address(LimitBuyHook::blueprint_id());

            // Create a resource manager to mint BuyOrder NFTs
            let orders_resource_manager = ResourceBuilder::new_integer_non_fungible_with_registered_type::<BuyOrderData>(
            ...
            Self {
                ...
                last_order_id: 0,
                order_books: KeyValueStore::new_with_registered_type(),
                         ...
                         // order can be immediately filled or both if case of a partial fill
        {
            ...
            let order = OrderRef::new(
                self.last_order_id,
                price,
                base_coin_bucket.amount(),
            );

            let mut order_book = self.order_books.get_mut(&coin_to_buy);
            match order_book {
                None => {
                    drop(order_book);
        ...
        pub fn withdraw(
            ...
            for order_nft in order_bucket.as_non_fungible().non_fungibles::<BuyOrderData>().iter() {

                let order_data = order_nft.data();

                let order_id = u32::try_from(
                ...
                let mut order_book = self.order_books.get_mut(&order_data.coin_to_buy).unwrap();

                let coin_amount = match coins_only {
                    true => order_book.withdraw_coins(&order_id),
                    false => {
                        let (base_coin_amount, coin_amount) = order_book.withdraw_all(&order_id);

                        base_coins_to_withdraw += base_coin_amount;
                ...
                if coin_amount > Decimal::ZERO {
                    ...
                    buckets.push(vault.take(coin_amount));
                }
            }
    ...
    impl HookInterfaceTrait for LimitBuyHook {
        ...
        fn hook(
            ...
            let order_book = self.order_books.get_mut(&argument.coin_address);
            ...
            // Fee adjusted number of coins in the pool
            let coin_in_pool = PreciseDecimal::from(pool_info.coin_amount) *
                (100 - pool_info.total_buy_fee_percentage) / 100;
            ...
            ) = order_book.as_ref().unwrap().find_matches(
                pool_info.base_coin_amount.into(),
                coin_in_pool,
            );
            ...
            order_book.unwrap().swap(filled_amount, coin_bucket.amount());
            ...
            Runtime::emit_event(
                MatchedOrderEvent {
                    ...
                    partially_filled_orders_id: partially_filled_order_id,
                }
            );
