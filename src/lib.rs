...
mod radix_pump {
    ...
    impl RadixPump {
        ...
        pub fn buy(
            ...
            let pool_base_coin_amount = PreciseDecimal::from(pool.base_coin_vault.amount());
            let pool_coin_amount = PreciseDecimal::from(pool.coin_vault.amount());
            let constant_product = pool_coin_amount * pool_base_coin_amount;
            let new_pool_coin_amount = constant_product / (pool_base_coin_amount + PreciseDecimal::from(base_coin_bucket.amount()));
            let coin_amount_bought = (pool_coin_amount - new_pool_coin_amount).checked_truncate(RoundingMode::ToZero).unwrap();
            ...
            Runtime::emit_event(
                BuyEvent {
                    ...
                    coins_in_pool: pool.coin_vault.amount(),
                }
            );

            pool.base_coin_vault.put(base_coin_bucket);

            pool.coin_vault.take(coin_amount_bought) // take_advanced ?
        }
        ...
        pub fn sell(
            ...
            let mut pool = self.pools.get_mut(&coin_bucket.resource_address()).expect("Coin not found");
            let pool_base_coin_amount = PreciseDecimal::from(pool.base_coin_vault.amount());
            let pool_coin_amount = PreciseDecimal::from(pool.coin_vault.amount());
            let constant_product = pool_coin_amount * pool_base_coin_amount;
            let new_pool_base_coin_amount = constant_product / (pool_coin_amount + PreciseDecimal::from(coin_bucket.amount()));
            let base_coin_bought_amount = (pool_base_coin_amount - new_pool_base_coin_amount).checked_truncate(RoundingMode::ToZero).unwrap();

            Runtime::emit_event(
                SellEvent {
                    resource_address: coin_bucket.resource_address(),
                    amount: coin_bucket.amount(),
                    price: base_coin_bought_amount / coin_bucket.amount(),
                    coins_in_pool: pool.coin_vault.amount(),
                }
            );

            pool.coin_vault.put(coin_bucket);

            let mut base_coin_bucket = pool.base_coin_vault.take(base_coin_bought_amount);
            self.fee_vault.put(
                base_coin_bucket.take_advanced(
                    base_coin_bucket.amount() * self.buy_sell_fee_percentage / dec!(100),
                    WithdrawStrategy::Rounded(RoundingMode::ToZero),
